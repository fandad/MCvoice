...
- 说话界面历史记录可点击再次朗读，默认停在最新一条，右侧带滚动条
- 滞留模式（默认关闭）：退出说话界面不丢草稿，发送后界面照常关闭
- 配置、高级设置、外部 TTS 服务、模型下载等可滚动界面右侧带滚动条
- 音量调节：支持 0%-200%
...
- 说话界面历史记录支持点击再次朗读，点击项用边框高亮，悬停显示完整文字。
- 新增滞留模式（默认关闭）：退出说话界面时未发送的文字保留，下次打开自动恢复；发送后界面照常关闭。
- 说话界面和各可滚动菜单右侧新增滚动条；历史默认显示最新一条，最旧的需要向上翻。
- 点击历史重读不会重复加入历史记录。
