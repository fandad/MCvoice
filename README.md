...
- 中英双语界面：配置、说话界面、聊天栏指令与全部悬浮注释都是双语的；英文界面下方有两行小灰字（一行说明界面本来只打算做中文，一行说明想用自己的模型或办法的人有充足的接口与内置引擎可用）
- 自动朗读开关：聊天框发送的文字可直接说出来，可用 B 键或 `/mcvoice auto` 快速切换
- 说话界面历史记录可点击再次朗读，默认停在最新一条，右侧带滚动条
- 滞留模式（默认关闭）：退出说话界面不丢草稿，发送后界面照常关闭
- 配置、高级设置、外部 TTS 服务、模型下载等可滚动界面右侧带滚动条，标题随内容一起滚动
- 声线选择界面：配置页点声线按钮打开列表选择；当前生效的那条灰显不可点，多音色模型分成多个条目
- 音量调节：支持 0%-200%
- 音频输出设置（高级设置 → 音频输出设置）：可把说话声另外送一路到指定输出设备，配合虚拟声卡（VB-CABLE / VoiceMeeter）让其他软件把它当麦克风用；三种输出方式（只在游戏内=默认 / 只送外部设备 / 两边都放）、外部那一路的独立音量（0-200%）、设备列表自动识别虚拟声卡并标注推荐置顶（注意：这一路音量主要跟随系统设置，界面上调整可能无效，界面里滑条下方也有同样提示）
...
- 支持 Sherpa-onnx 离线中文模型，提供更多本地声线，含粤语"小美"、Kokoro 中英多音色（8 个中文音色）、Matcha Baker
- 多说话人模型（寒冰、伊拉、小爱风格）可展开为多条独立声线，共 12 条
...
在“下载中文声线”页面的 Sherpa / Matcha / Kokoro 区块下载模型。模组会自动解压到 `mcvoice/models/sherpa`，无需手动处理压缩包；下载支持断点续传，中断后点一次按钮即可从断点继续。
...
当前一键下载包含：MeloTTS 中英女声、寒冰、伊拉、繁辰 WNJ、小爱风格、超文、小雅、**小美（粤语）**、**Matcha Baker**、**Kokoro 多语言**。超文和小雅来自 `phoneme_type=pinyin` 的 Piper 模型，但以 Sherpa-onnx 打包格式提供，不需要单独安装 g2pW 运行时。

小美（粤语）约 108MB、Matcha Baker 约 72MB、Kokoro 约 126MB，均来自 sherpa-onnx 官方模型库，下载不需要账号或登录。模型自带 `rule.fst` 时会自动加载，用于改善数字和日期的朗读。

寒冰、伊拉、小爱风格、Kokoro 都是单模型多说话人：模组会把选定的说话人展开成多条声线（可在声线选择界面分别选择），因此只需下载一次。

> 提示：部分模型的下载源在国内不稳定，下载页顶部有黄色提醒，建议挂加速器（加速 GitHub 等）。
...
游戏内播放只有你自己听得到。如果想让别的软件（KOOK / OOPSZ / Discord、录制或直播工具）把你说的内容当成麦克风输入，需要一个虚拟声卡 —— Windows 的"录音设备"必须由驱动提供，程序本身无法凭空出现在麦克风列表里。

1. 安装虚拟声卡（推荐 VB-CABLE，免费）：解压后**以管理员身份**运行 `VBCABLE_Setup_x64.exe`，装完**重启电脑**（安装时窗口可能显示"未响应"几秒，属正常）。
2. 游戏里打开 高级设置 → 音频输出设置，输出方式选「只送外部设备」（自己不听）或「游戏内 + 外部设备」（两边都听）。
3. 设备列表里选虚拟声卡的**输入端**（例如 `扬声器 (2- VB-Audio Virtual Cable)` 或 `CABLE In 16 Ch (2- VB-Audio Virtual Cable)`）——它会被自动标注"虚拟麦克风（推荐）"并排在最前。
4. 语音软件里把麦克风 / 输入设备选成虚拟声卡的**输出端**，例如 `CABLE Output (2- VB-Audio Virtual Cable)`。
5. 对方听不到时依次检查：Windows 设置 → 隐私和安全性 → 麦克风 → 允许桌面应用访问麦克风；第 4 步是否选的是本机实际存在的那个端点（本机有两个同名端点，能用的那个带 `2-` 前缀）。

外部那一路的音量由「外部设备音量」单独控制，与主音量相互独立；主音量同时影响两路。**这一路音量主要跟随系统设置，在这里调整可能无效**（界面里滑条下方也有这条提示）。输出方式默认是「只在游戏内」，此时模组完全不会打开任何输出设备。
...
- 新增英文界面语言文件（`en_us.json`），原先把中文硬编码在代码里的界面文案全部改为翻译 key；两个语言文件现在各 227 个键。非中文客户端不再显示裸键名；英文界面下方有两行小灰字（界面本来只打算做中文 / 有自己的模型或办法的人有充足接口与内置引擎可用）。
- 音频输出界面的外部音量滑条下方新增提示：这一路音量主要跟随系统设置，在这里调整可能无效。
- 新增音频输出设置：可把说话声另外送一路到指定输出设备，配合虚拟声卡让其他软件当成麦克风输入；含三种输出方式、外部那一路的独立音量、自动识别并置顶虚拟声卡的设备列表（详见上文「把声音送进语音软件」）。
- 修复说话卡顿：播放循环原来"做完工作再固定睡 20ms"，每帧耗时超过音频本身的 20ms，音频引擎持续欠载；现已改为按绝对时间对齐（实测每帧 26.7ms → 20.0ms）。
- 修复游戏内完全听不到自己：Simple Voice Chat 的音量分类注册失败会中断连接回调，导致本地播放通道建不起来。
- 修复选中的输出设备不一定真的收到声音（Java Sound 全局取线顺序问题）。
- 选择"只送外部设备"而未选设备或设备打不开时，改为自动回退游戏内播放，不再彻底静音。
- 输出方式默认「只在游戏内」，默认状态下不会打开任何输出设备。
- 修复：Piper 的两条声线（花颜女声 / 花颜低配）在声线选择界面显示为原始模型 id，现在正确显示中文名。

### 0.2.6

- 新增声线选择界面：配置页的声线按钮改为打开列表，当前生效的那条灰显不可点，多音色模型分成多个条目。
- Sherpa 下载区新增 Kokoro 多语言（约 126MB，8 个中文音色）与 Matcha Baker（约 72MB）。
- 寒冰、伊拉、小爱风格展开为多条独立声线，共 12 条。
- Sherpa 下载支持断点续传；请求超时从 20 分钟提高到 30 分钟；下载失败写入日志。
- 下载页重排为上下两块并按两列网格对齐，顶部新增加速器提示，补充滚动条并修复滚动范围。
- 可滚动页面的标题随内容一起滚动。
- 1.21.8 不再部署进游戏目录，jar 仍构建与归档。
...
```text
mc26/build/libs/mcvoice-0.2.7+26.x.jar
mc12111/build/libs/mcvoice-0.2.7+1.21.11.jar
mc1218/build/libs/mcvoice-0.2.7+1.21.8.jar
mc1211/build/libs/mcvoice-0.2.7+1.21.1.jar
```

`mcvoice-0.2.7+26.x.jar` 覆盖 26.1、26.1.1、26.1.2 和 26.2；`1.21.11`、`1.21.8`、`1.21.1` 各自独立。
