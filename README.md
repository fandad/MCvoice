...
- 中英双语界面：配置、说话界面、聊天栏指令与全部悬浮注释都是双语的；英文界面下方有一行灰字说明本模组以中文为主
...
- 音量调节：支持 0%-200%
- 音频输出设置（高级设置 → 音频输出设置）：可把说话声另外送一路到指定输出设备，配合虚拟声卡（VB-CABLE / VoiceMeeter）让其他软件把它当麦克风用；三种输出方式（只在游戏内=默认 / 只送外部设备 / 两边都放）、外部那一路的独立音量（0-200%）、设备列表自动识别虚拟声卡并标注推荐置顶
...
外部那一路的音量由「外部设备音量」单独控制，与主音量相互独立；主音量同时影响两路。输出方式默认是「只在游戏内」，此时模组完全不会打开任何输出设备。
...
- 新增英文界面语言文件（`en_us.json`），原先把中文硬编码在代码里的界面文案全部改为翻译 key；两个语言文件现在各 225 个键。非中文客户端不再显示裸键名，英文界面下方另有一行灰字说明本模组以中文为主。
